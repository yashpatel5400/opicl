...
class SpectralConvolution(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        Args:
            x (torch.Tensor): Input tensor of shape (batch, in_channels, D1, D2, ..., DN, T).

        Returns:
            torch.Tensor: Output tensor of shape (batch, out_channels, D1, D2, ..., DN, T).
        """
        ...
        with amp.autocast('cuda', enabled=False):
            x_ft = torch.fft.fftn(x.float(), dim=tuple(range(-(self.dim+1), -1)), norm='ortho')
        ...
        out = torch.fft.ifftn(out_ft, dim=tuple(range(-(self.dim+1), -1)), s=sizes[:-1], norm='ortho').real

        # Add bias if present
        if self.bias is not None:
            out = out + self.bias.view(1, -1, *([1] * (self.dim+1)))
